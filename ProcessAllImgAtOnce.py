...
PATH_TO_LABELS = os.path.join('training', 'labelMap.pbtxt')  # our labels are in training/object-detection.pbkt
...
TEST_IMAGE_PATHS = [os.path.join(PATH_TO_TEST_IMAGES_DIR, 'image{}.jpg'.format(i)) for i in range(5)]  # adjust range for # of images in folder
...
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        ...
        for image_path in TEST_IMAGE_PATHS:
            ...
            width2 = width
            height2 = height
            while width2 >= 720 or height2 >= 500:
                resize(image_path)
                image = Image.open(image_path)
                width, height = image.size
                width2 = width
